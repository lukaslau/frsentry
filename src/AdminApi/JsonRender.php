...
class JsonRender
{
    ...
    public function render(array $data, int $code = self::HTTP_OK)
    {
        if (!isset(self::$statusTexts[$code])) {
            $code = 500; // gdy jakis exception nie jest lapany to go nam pokaze w responsie dzieki temu
        }
        ...
        header(sprintf('HTTP/%s %s %s', '1.1', $code, self::$statusTexts[$code]), true, $code);
